...
const {validateCreateUser, validateUpdateApartOfUser,
    validateUpdateLicensePlates, validateUserId} = require('../../services/validation/validationUser');
//GET
...
module.exports.updateApartOfUser = async (req, res, next) =>{
    try {
        ...
        const valid = await validateUpdateApartOfUser(req.body);
        if(valid.error){
            console.log(valid.error);
            res.status(400).json({message: "Parameter incorrect!"});
        }else{
...
module.exports.updateLicensePlates = async (req, res, next) =>{
    try {
        ...
        const valid = await validateUpdateLicensePlates(req.body);
        if(valid.error){
            console.log(valid.error);
            res.status(400).json({message: "Parameter incorrect!"});
        }else{
